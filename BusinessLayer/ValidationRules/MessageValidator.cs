...
namespace BusinessLayer.ValidationRules
{
    class MessageValidator: AbstractValidator<Message>
    {
        public MessageValidator()
        {
            ...
            RuleFor(x => x.ReceiverMail).MinimumLength(3).WithMessage("Lütfen en az 3 karakter giriniz");
            RuleFor(x => x.Subject).MinimumLength(3).WithMessage("Lütfen en az 3 karakter giriniz");
            RuleFor(x => x.Subject).MinimumLength(3).WithMessage("Lütfen 100 karakterden fazla değer girişi yapmayın");
        }
    }
}
